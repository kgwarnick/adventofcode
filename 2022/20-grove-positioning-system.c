...
/// \brief Do one or more mixes of the numbers on the input lines
//
long RunMixer (size_t numlines, const char **lines, int nummix) {
  Number numbers[numlines];
  for (size_t i = 0; i < numlines; i++) {
    numbers[i].value = strtol (lines[i], NULL, 10);
    numbers[i].prev = &(numbers[i > 0 ? i - 1 : numlines - 1]);
    numbers[i].next = &(numbers[i < numlines - 1 ? i + 1 : 0]);
  }
  for (int nmix = 0; nmix < nummix; nmix++) {
    for (size_t i = 0; i < numlines; i++) {
      // printf ("%4zu  %4d <- %4d -> %4d\n", i, numbers[i].prev->value,
      //   numbers[i].value, numbers[i].next->value);
      ...
      // printf ("thisnum: %d -> prev %d, next %d;  newpos: %d -> %d, %d\n",
      //   thisnum->value, thisnum->prev->value, thisnum->next->value,
      //   newpos->value, newpos->prev->value, newpos->next->value);
      ...
      //   printf ("%4zu  %4d <- %4d -> %4d\n", j, numbers[j].prev->value,
      //     numbers[j].value, numbers[j].next->value);
      // }
...
int main () {
  ...
  result = RunMixer (numlines, (const char**)inputlines, 1);
  printf ("*** Result: %ld ***\n", result);
  return 0;
}
